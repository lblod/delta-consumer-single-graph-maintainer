...
async function remapFiles() {
  ...
  for (let zip of vuuidsAndFilenames) {
    try {
      ...
      let newuri = await remapURI(zip.vuuid.value);

      //Only if a new uri actually exists
      if (newuri) {
        //Move file to new location in accordance to the new URI
        await moveFileToURI(zip.pfilename.value, newuri);
      }
...
async function moveFileToURI(filename, newuri) {
  let oldfullpath = path.join("/share/", filename);
  let newfullpath = newuri.replace("share://", "/share/");
  return moveFile(oldfullpath, newfullpath);
}
...
async function updatePURI(olduri, newuri) {
  ...
  `;
  try {
    let res = await updateSudo(queryString);
    return res;
  }
  catch (err) {

...
import * as tripleDispatching from "../../dispatch-extensions/index.js";
  ...
  async ingest(onFinishCallback) {
    ...
    try {
      ...
      for (let changeSet of changeSets) {
        changeSet = await performChangeSetHooks(changeSet);
        ...
        //Store regular triples in their dispatched graph
        console.log(`Inserting regular triples`);
        await processInserts(regularInserts);
        console.log("Deleting triples from all graphs");
        ...
        if (PERFORM_SEPERATE_FILE_SYNC) {
          //Dispatch and convert triples into metadata, because of the buffering for files
          ...
          await insertTriplesInGraph(fileInsertsMeta, TEMP_FILE_GRAPH, MU_CALL_SCOPE_ID_FILE_SYNC);
          await insertTriplesInGraph(fileDeletesMeta, TEMP_FILE_REMOVAL_GRAPH, MU_CALL_SCOPE_ID_FILE_SYNC);
        }
        else {
          await processInserts(fileInserts);
          await processDeletes(fileDeletes);
        }
      }
...
function performChangeSetHooks(triples) {
  //TODO Throw the triples at some extensions to dispatch them to a correct graph. This changes their graph definition.
  return tripleDispatching.performHooks(triples);
}

async function processInserts(triples, scope) {
  //Collect triples per graph and insert in those graphs
  let grouping = {};
  for (const triple of triples) {
    if (grouping[triple.graph.value])
      grouping[triple.graph.value].push(triple);
    else
      grouping[triple.graph.value] = [triple];
  }
  for (const graphGroup in grouping) {
    await insertTriplesInGraph(grouping[graphGroup], graphGroup, scope);
  }
}

  ...
  async ingest(onFinishCallback) {
    ...
    try {
      ...
      for (let {inserts, deletes} of changeSets) {
        //Filter on file triples
        let partitionedInserts = partition(inserts, change => change.graph.value === UPLOADED_FILES_GRAPH);
        let partitionedDeletes = partition(deletes, change => change.graph.value === UPLOADED_FILES_GRAPH);
        let regularInserts     = partitionedInserts.fails;
        let regularDeletes     = partitionedDeletes.fails;
        let fileInserts        = partitionedInserts.passes;
        let fileDeletes        = partitionedDeletes.passes;
        ...
        //console.log("WHILE INGESTING, this is the partioning of inserts: ", JSON.stringify(partitionedInserts));
        //console.log("WHILE INGESTING, this is the partioning of deletes: ", JSON.stringify(partitionedDeletes));
        await insertTriplesInGraph(partitionedInserts.passes, TEMP_FILE_GRAPH);
        await deleteTriplesInAllGraphs(partitionedDeletes.passes);
      }

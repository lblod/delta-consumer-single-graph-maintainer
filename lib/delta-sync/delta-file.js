...
import { createTargetDir } from '../utils.js';
...
export default class DeltaFile {
  constructor(data) {
    ...
    this.created = new Date(data.attributes.created);
   ...
   */
  get deltaFilePath() {
    //Specify a subfolder based on the this.created
    const subFolder = `${this.created.getFullYear()}-${this.created.getMonth() + 1}-${this.created.getDate()}`;
    return path.join(DELTA_FILE_FOLDER, subFolder, `delta-${this.created.toISOString()}-${this.id}.json`);
  }
  ...
  async consume(onFinishCallback) {
    //Ensure target directory first, which might not fully exist yet
    await createTargetDir(this.deltaFilePath);

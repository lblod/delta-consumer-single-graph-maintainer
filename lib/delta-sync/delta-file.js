    ...
    try {
      ...
      for (let {inserts, deletes} of changeSets) {
        ...
        if (PERFORM_SEPERATE_FILE_SYNC) {
          await insertTriplesInGraph(fileInserts, TEMP_FILE_GRAPH, MU_CALL_SCOPE_ID_FILE_SYNC);
          await insertTriplesInGraph(fileDeletes, TEMP_FILE_REMOVAL_GRAPH, MU_CALL_SCOPE_ID_FILE_SYNC);
        }
        else {
          await insertTriplesInGraph(fileInserts, INGEST_GRAPH);
          await insertTriplesInGraph(fileDeletes, INGEST_GRAPH);
        }
      }

...
async function dispatch(lib, data){
  ...
  console.log(`Found an amount of ${changeSets.length} changesets`);
  for (let { deleteTermObjects, insertTermObjects } of changeSets) {
    console.log(`Logging delete information: `);
    const deleteStatements = deleteTermObjects.map(o => `In graph: ${o.graph}, triple: ${o.subject} ${o.predicate} ${o.object}`);
    deleteStatements.forEach(s => console.log(s));

    console.log(`Logging insert information: `);
    const insertStatements = insertTermObjects.map(o => `In graph: ${o.graph}, triple: ${o.subject} ${o.predicate} ${o.object}.`);
